    ...
    public Display(GOLDParser parser) {
        ...
        if (reduction != null) {
            if (reduction.size() == 2) {
                ...
                if (reduction.size() < 3) {
                    // print x form
                    parser.raiseParserException(FormatHelper.formatMessage("error.param_count", "2", String.valueOf(reduction.size())));
                }
                // print x read y form
                parser.raiseParserException(FormatHelper.formatMessage("error.param_count", "4", String.valueOf(reduction.size())));
            }
        } else {
