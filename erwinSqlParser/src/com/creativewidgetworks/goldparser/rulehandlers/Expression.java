...
public class Expression extends Reduction {
        ...
        if (reduction != null) {
            if (reduction.size() == 3) {
                ...
                if (validOperators.indexOf(theOperator + " ") == -1) {
                    parser.raiseParserException(FormatHelper.formatMessage("error.invalid_operator", validOperators, theOperator));
                }
            } else {
                parser.raiseParserException(FormatHelper.formatMessage("error.param_count", "3", String.valueOf(reduction.size())));
            }
        } else {
         //   parser.raiseParserException(FormatHelper.formatMessage("error.no_reduction"));
        }        
    }
    ...
    public Variable getValue() throws ParserException {
        ...
        if (theOperator.equals("==")) {
           ...
           //    theParser.raiseParserException(FormatHelper.formatMessage("error.type_mismatch"));
           }
